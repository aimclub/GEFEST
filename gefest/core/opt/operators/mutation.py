...
def mutation(structure: Structure,  domain: Domain, min_point_num, max_point_num, rate=0.6):
    ...
    while not is_correct and n_iter < MAX_ITER:
        ...
        if NUM_PROC > 1:
            with Pool(NUM_PROC) as p:
                new_items = \
                    p.map(mutate_worker,
                          [[new_structure, changes_num, min_point_num, domain] for _ in range(NUM_PROC)])
        else:
            new_items = [mutate_worker([new_structure, changes_num, min_point_num, max_point_num, domain]) for _ in range(NUM_PROC)]
...
def mutate_worker(args):
    structure, changes_num, min_point_num, max_point_num, domain = args[0], args[1], args[2], args[3], args[4]
    ...
    try:
        ...
        for _ in range(changes_num):
            ...
            if random.random() < polygon_drop_mutation_prob and len(new_structure.polygons) > 1:
                ...
                new_poly = get_random_poly(min_point_num=min_point_num,
                ...
                if (random.random() < point_drop_mutation_prob and
                        len(polygon_to_mutate.points) > min_point_num):
