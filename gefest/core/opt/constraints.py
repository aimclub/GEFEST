...
#     is_contain
from gefest.core.structure.domain import *


class Constraints:
    @classmethod
    def check_constraints(
        cls, structure: Structure, is_lightweight: bool = False, domain=None, model_func=None
    ) -> bool:
        try:
            if any(
                [
                    (
                        poly is None
                        or len(poly.points) == 0
                        or any([pt is None for pt in poly.points])
                    )
            ...
            cts = [
                cls.out_of_bound(structure, domain),
                cls.too_close(structure, domain),
                cls.is_contain(structure, domain),
                cls.self_intersection(structure),
                cls.intersection(structure, domain),
                cls.unclosed_poly(structure, domain),
            ]
        ...
        return any(
            [
                ...
                and _forbidden_validity(
                    explain_validity(GeomPolygon([GeomPoint(pt.x, pt.y) for pt in poly.points]))
                )
                for poly in structure.polygons
            ]
        )
