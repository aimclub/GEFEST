...
tuned_2 =  parse_structs(f'{pr_root}/cases/sound_waves/logs/run_name_2023-10-11_12_58_51/00100.log')[-1]
tuned_1 =  parse_structs(f'{pr_root}/cases/sound_waves/logs/run_name_2023-10-11_15_25_40/00100.log')[-1]
...
plt.plot([p.coords[0] for p in tuned_p],[p.coords[1] for p in tuned_p],label=f'notTuned_1,fitness :{tuned_1.fitness[0]}')
plt.plot([p.coords[0] for p in tuned_2_p],[p.coords[1] for p in tuned_2_p],label=f'notTuned_2,fitness :{tuned_2.fitness[0]}')
plt.plot([x[0] for x in [i.coords for i in best_structure.polygons[0].points]],[x[1] for x in [i.coords for i in best_structure.polygons[0].points]],label='Init fig')
plt.legend()
plt.show()
print()